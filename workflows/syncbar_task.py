...
def sync_symbol_date(base_symbol: str, sync_symbol: str, date: str, config_id: str):
    # get base symbol bars
    base_bars = data_access.fetch_sd_data(base_symbol, date, prefix=f"/bars/{config_id}/df")
    ...
    for bar in base_bars.itertuples():
        ...
        if len(syncbar_ticks) < 1:
            print(base_symbol, sync_symbol, date, 'zero tick syncbar')
            continue
    ...
    print(sync_symbol, date)  # logging
    data_access.presist_sd_data(pd.DataFrame(sync_bars).reset_index(drop=True),
        symbol=sync_symbol,
        date=date,
        prefix=f"/bars/{config_id}/sync_bars/clock_symbol={base_symbol}",
        destination='local'
        )
